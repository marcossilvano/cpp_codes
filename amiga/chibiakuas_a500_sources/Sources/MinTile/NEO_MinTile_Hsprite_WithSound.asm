...
;Interrupt Vectors
	dc.l $00C00432		;$00000060: Virtual Interrupt (Spurious Interrupt)
	dc.l VBlank			;$00000064: Level 1 interrupt (VBlank)
	dc.l IRQ2			;$00000068: Level 2 interrupt (HBlank)
	dc.l IRQ3			;$0000006C: Level 3 interrupt
	dc.l $00000000		;$00000070: Level 4 interrupt
	dc.l $00000000		;$00000074: Level 5 interrupt
	dc.l $00000000		;$00000078: Level 6 interrupt
	dc.l $00000000		;$0000007C: Level 7 interrupt (NMI)
...
USER:	
	...
	;SR:  T-S--III---XNZVC 	S=Supervisor / I=Interrupt level
	move.w	#$2000,sr			;Supervisor mode / Interrupt level 0 
...
VBlank:
	btst #7,$10FD80			;BIOS_SYSTEM_MODE 
	bne InGameMode			;1=Game mode 0=System mode
		jmp $C00438			;SYSTEM_INT1 - run BIOS vblank
		
InGameMode:					;Run our vblanks
	...
	movem.l d0-d7/a0-a6,-(sp) 
		...
		jsr ChibiTracks_Play ;Update music
			...
			;%------VH3		3=Irq3 H=Hblank V=Vblank
		move.w #4,$3C000C	;LSPC_IRQ_ACK - Interrupt clear reg. 
		
		move.b d0,$300001	;REG_DIPSW - kick the watchdog	
		
		jsr	$C0044A 		;SYSTEM_IO - Sould execute every 1/60secs
		jsr	$C004CE			;MESS_OUT  - Display system messages
...
;Init Sound
	...
	jsr chibisoundpro_init		;Init platform drver
	
	move.l #Song1,a3			;Song address
	move.l a3,(SongBase)
	
	jsr StartSong 				;Load song settings
...
;;;;;;;;;;;; Define TileMap ;;;;;;;;;;;; 
	...
	;move.w #$0FFF,d5 		;Scale (16x16)
	move.w #$077F,d5 		;Scale (8x8)
...
TileLoop:
	move.w d6,$3C0000 	
	move.w #$2200,$3C0002	;Tile Number (defaults to $2010)
	...
	dbra d3,TileLoop		;Repeat for next 
