...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;	
;A2=Tilemap A5=TileOffset D1,D4=x,y pos D7=Width	
...
NextTileSpr:
	...
	NoClearSpr:
		...
		move.l d0,-(sp)
			...
			clr.l d0
			move.b d4,d0			;Ypos in pairs of pixels
			asl.l #1,d0				;*2
			add.l #128+(2*8),d0
			...
			move.w d0,(VDP_data)	; ----WWHH -LLLLLLL 
									;Width, Height, Link (to next sprite)
			
		move.l (sp)+,d0			;Tilenum
		add.l a5,d0
		move.w d0,(VDP_data)	; PCCVHNNN NNNNNNNN - Priority, Color palette
									;Vflip, Hflip, tile Number
		clr.l d0
		move.b d1,d0			;Xpos in pairs of pixels
		asl.l #1,d0				;*2
		add.l #128+(4*8),d0
...
FinishSprites:
	jsr GetHSprite			;Next Sprite
;Loop back to start of sprites
	move.w #0,(VDP_data)	; ------VV VVVVVVVV - Vpos
	move.w #0,(VDP_data)	; ----WWHH -LLLLLLL - Width, Height, Link 
													;(to next sprite)
	move.w #0,(VDP_data)	; PCCVHNNN NNNNNNNN - Priority, Color palette 
												  ;Vflip, Hflip, tile Number
...
GetScreenPos: 
	...
	move.l d1,d0			;Xpos
	and.l #%11111100,d0		;Round to tilea	
	asr.l #1,d0				;Xpos in tiles *2
	add.l d0,a6
	
	move.l d4,d0			;Ypos
	and.l #%11111100,d0
	asl.l #5,d0				;Ypos in tiles *64
	add.l d0,a6
	rts
