...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	...
	jsr chibisoundpro_init	;Init platform sound driver
	
	move.l #Song1,a3		;Set song address
	move.l a3,(SongBase)
	
	jsr StartSong 			;Load in song data
	
	move.b #6,(SongSpeed)	;Tweak speed for 60hz
	
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

;Lets set the address of our interrupt vectors...
;Top 4 bits of Vector number (Vector Address*4)
		;   %VVVVA--- 
	move.b #$40,d0			;$000100 (Vector $40 - Default)
	move.b d0,$e88017
						
	
;There are 16 Interrupts from the MFP, from vector $40 (address $000100+)
;We want GPIP4 - which uses interrupt 6

;$000100+4*6	UserInterrupt: MFP V-DISP1(Vblank / GPIP4)
	move.l #InterruptHandler,$000118 
	
;First lets disable all MFP interrupts except Vblank	
			;%76AFRETB	7=Gpip7 6=Gpip6 A=Timer A F=recieve buffer Full 
						;R=Recieve error E=send buffer Empty 
						;T=transmit error B=timer B
	move.b #%00000000,$e88007	;Interrupt Enable
	
		   ;%54CD3210	5=gpip 5 4=gpip4 C=timer C D=timer 
						;D 3=gpip3 2=gpip2 1=gpip1 0=gpip 0
	move.b #%01000000,$e88009
	
;Now lets' set the mask too, just to be sure!
	move.b #%00000000,$e88013	;Masks
	move.b #%01000000,$e88015
...
chibisoundram:
		ds 256			;Define some memory for ChibiSound
...
InterruptHandler:
	movem.l d0-d7/a0-a6,-(sp)	;save registers
		...
		jsr ChibiTracks_Play	;Update the music
		
	movem.l (sp)+,d0-d7/a0-a6	;restore registers
	
	rte							;Return from interrupt handler
