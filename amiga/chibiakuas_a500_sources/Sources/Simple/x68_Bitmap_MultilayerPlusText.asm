...
; Layer order
	...
	move.w #%0000111011100100,$e82500 ;R1 (Priority control) - Priority
...
;Turn on layers 0-3
	...
	move.w #%0000000011101111,$e82600 ;R2 (Special priority/screen display)
										;Screen On - sprites on - Text on
...
;Proper Chibiko colors for text layer!
	move.w #%0000000000000000,$e82200	;Color 0
	move.w #%0000001110011100,$e82202	;Color 1
	move.w #%0111100000011110,$e82204	;Color 2
	move.w #%0111101111011110,$e82206	;Color 3
...
;The text screen is still operational	
	move.w #'Z',-(a7)		;Char to print
	dc.w $FF02        	;_PUTCHAR - Show Character to text layer!!
	
;Hide os screen objects.

;Cursor off
	move.w #$12,-(a7)	;_CONCTRL - Console Control $12=Cursor Off
	dc.w $FF23        	;
	addq.l  #2,sp
	
;CLS
	move.w #$02,-(a7)	;Mode=2 - Whole screen
	move.w #$0A,-(a7)	;_CONCTRL - Console Control $0A=CLS
	dc.w $FF23        	;
	addq.l  #4,sp

;Function keys off
	move.w #$03,-(a7)	;Mode=3 - No Function Key Display
	move.w #$0E,-(a7)	;_CONCTRL - Console Control $0E=FuncKeys
	dc.w $FF23
	addq.l  #4,sp
...
;Draw to the 4 bitmap layers
	...
	move.w #128-24,d1		;x
	move.w #128-24,d2		;y
	jsr GetScreenPosTxt		;Get Position in Vram
	jsr ShowTextBmp			;Draw to Text layer
...
InfLoop
	...
	;move.w d1,$e80014		;Text-X
	move.w d2,$e80016		;Text-Y
...
ShowBmp:	
	...
	lea Bitmap,a0
BmpNextLine:			
	move.l #(48/2)-1,d1		;2 pixels per Byte in 16 color mode
	move.l a6,-(sp)
BmpNextPixel:
...
GetScreenPosTxt: ; d1=x d2=y result in A3
	moveM.l d0-d7/a0-a2,-(sp)
		
		and.l #$3FF,d1
		and.l #$3FF,d2
		
		lsr.l #3,d1				;8 pixels per byte
		add.l #$E00000,d1		;Text Bitplane 0 address
		move.l d1,a3
		
		lsl.l #7,d2				;128 bytes per Y line 
		add.l d2,a3
...
ShowTextBmp:
	move.l a3,a4
	add.l #$20000,a4		;Bitplane 1 address
	move.l a3,a5
	add.l #$40000,a5		;Bitplane 2 address
	move.l a3,a6
	add.l #$60000,a6		;Bitplane 3 address
	
	move.l #48-1,d2			;Height
	lea TextBitmap,a0
ShowTextBmpNextLine:			
	move.l #(48/8)-1,d1		;4 pixels per word in 8 color mode
	moveM.l a3-a6,-(sp)
ShowTextBmpNextPixel:		
		move.b (a0)+,(a3)+	;Bitplane 0
		move.b (a0)+,(a4)+	;Bitplane 1
		move.b (a0)+,(a5)+	;Bitplane 2
		move.b (a0)+,(a6)+	;Bitplane 3
		
		dbra d1,ShowTextBmpNextPixel
	moveM.l (sp)+,a3-a6		;Get the left Xpos back
	addA #128,a3			;Move down a line B0
	addA #128,a4			;Move down a line B1
	addA #128,a5			;Move down a line B2
	addA #128,a6			;Move down a line B3
	dbra d2,ShowTextBmpNextLine
	rts
