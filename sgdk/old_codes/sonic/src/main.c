...
//fix32 enemiesPosX[ENEMY_MAX];
//fix32 enemiesPosY[ENEMY_MAX];
//s16 enemiesXOrder[ENEMY_MAX];

// animation index table for enemies (static VRAM loading)
u16** sprTileIndexes[ENEMY_MAX];
// BG start tile index
...
static void initEnemies() {
    ...
    for (u16 i = 0; i < ENEMY_MAX; i++) {
        // random()
        // definindo um layout de matriz para inimigos
        enemies[i].x = FIX32((i % ENEMY_COLS) * 50 + 50); 
        enemies[i].y = MAX_POSY - 250 - FIX32((i / ENEMY_COLS) * 30);
        enemies[i].xOrder = -1;       
        enemies[i].sprite = SPR_addSprite(&megaman, fix32ToInt(enemies[i].x) - camPosX, fix32ToInt(enemies[i].y) - camPosY, 
                            TILE_ATTR(PAL0, TRUE, FALSE, FALSE));

        // disable auto tile upload for enemies sprites as we will pre-load all animation frams in VRAM for them
        SPR_setAutoTileUpload(enemies[i].sprite, FALSE);
        // set frame change callback for enemies so we can update tile index easily
        SPR_setFrameChangeCallback(enemies[i].sprite, &frameChanged);
        // store enemy 'sprTileIndexes' table index in 'data' field (can be used freely)
        enemies[i].sprite->data = 0;
    }

    u16 numTile;
    // pre-load all animation frames into VRAM for enemies
    sprTileIndexes[0] = SPR_loadAllFrames(&megaman, ind, &numTile); //&enemy01_sprites
    ind += numTile;
...
//    memcpy(&palette[48], enemies_sprite.palette->data, 16 * 2);
    ...
    // fade in
    PAL_fadeIn(0, (4 * 16) - 1, palette, 1, FALSE);
...
static void updatePhysic()
{
    ...
    // set enemies sprite position
    for (u16 i = 0; i < ENEMY_MAX; i++) {
        setSpritePosition(enemies[i].sprite, fix32ToInt(enemies[i].x) - camPosX, fix32ToInt(enemies[i].y) - camPosY);
    }
}
...
//    else enemiesPosX[1] -= FIX32(0.7);
    for(u16 i = 0; i < ENEMY_MAX; i++)
    {
        ...
        if (enemies[i].xOrder > 0) 
            enemies[i].x += FIX32(ENEMY_SPEED);
        else 
            enemies[i].x -= FIX32(ENEMY_SPEED);
            
//        enemies[i].x += FIX32(ENEMY_SPEED);

        if ((enemies[i].x >= FIX32(320L - 50L)) || (enemies[i].x <= MIN_POSX))
            enemies[i].xOrder = -enemies[i].xOrder;
...
/*
    ...
    {
        if (enemiesXOrder[i] > 0) 
            SPR_setHFlip(enemies[i], TRUE);
        else 
            SPR_setHFlip(enemies[i], FALSE);
    }
*/
}
